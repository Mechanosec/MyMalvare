...
Після перерваного запуску можна продовжити з останньої **повністю збереженої** епохи:

```bash
localModel/.venv/bin/python -m localModel.train_head \
  --data-dir localModel/.cache/evaluation \
  --out-dir localModel/.cache/checkpoints \
  --resume-from localModel/.cache/checkpoints/epoch-02
```

Перед продовженням перевіряються хеш ваг, маніфест даних, seed, інструкція моделі й адаптер. Ваги продовжуються, але моменти AdamW після перезапуску обнуляються; це записується в `training.json`. Якщо потрібен повністю тотожний безперервному навчанню експеримент, почніть у новому каталозі з базових ваг.
