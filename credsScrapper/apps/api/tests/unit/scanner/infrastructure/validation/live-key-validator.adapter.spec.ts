...
describe('LiveKeyValidatorAdapter', () => {
  ...
  it.each([
    ['ENOTFOUND', 'Inconclusive: provider hostname could not be resolved.'],
    ['ECONNRESET', 'Inconclusive: connection to provider failed.'],
    [
      'CERT_HAS_EXPIRED',
      'Inconclusive: provider TLS certificate could not be verified.',
    ],
    ['UND_ERR_CONNECT_TIMEOUT', 'Inconclusive: provider request timed out.'],
    [
      'ERR_INVALID_CHAR',
  ...
  it('reports timeout without including error text or credentials', async () => {
    global.fetch = jest
      .fn()
      .mockRejectedValue(new DOMException('sensitive-fixture', 'TimeoutError'));
    expect(
  ...
  it('returns INVALID for a Telegram token that 401s', async () => {
    global.fetch = jest
      .fn()
      .mockResolvedValue({ ok: false, status: 401 }) as never;
  ...
  it('returns INVALID for a Stripe test/restricted key that 401s', async () => {
    ...
    ).toBe(EFindingStatus.INVALID);
    expect(
      await adapter.validate(ESecretType.STRIPE_RESTRICTED_KEY, 'rk_live_fake'),
    ).toBe(EFindingStatus.INVALID);
  });
  ...
  it('returns VALID for both a DigitalOcean PAT and OAuth token via the same /v2/account check', async () => {
    ...
    expect(
      await adapter.validate(ESecretType.DIGITALOCEAN_PAT, 'dop_v1_fake'),
    ).toBe(EFindingStatus.VALID);
    expect(
      await adapter.validate(
  ...
  it('returns VALID for an AWS credential pair that sts:GetCallerIdentity confirms with 200', async () => {
    ...
    expect(options.headers.Authorization).toMatch(
      /^AWS4-HMAC-SHA256 Credential=AKIAABCDEFGH12345678\//,
    );
