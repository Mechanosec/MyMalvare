...
describe('GetFindingsRepoOptionsUseCase', () => {
  ...
  it('reports failed checks separately in repository options', async () => {
    const state = new FakeStateRepository();
    await state.addFinding(
      1,
      'fixture',
    ...
    await state.recordTestResult(0, EFindingStatus.FAILED);

    const options = await new GetFindingsRepoOptionsUseCase(state).execute();
