...
2. **New port (if needed)** — `application/ports/<name>.port.ts`: an abstract class with abstract methods and no implementation; the class itself is the NestJS DI token (follow existing `*.port.ts` files).
...
After wiring, read and apply `../hex-architecture-reviewer/SKILL.md` to catch layer violations before finishing. Run the applicable checks in `credsScrapper/AGENTS.md`.
